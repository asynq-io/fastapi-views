...
- **`Translatable[T]`** — a Pydantic field type that holds a translation key and resolves
...
common internal `_DictTranslationManager` base that provides:
...
lookup. `Translatable[T]` is an annotated string type that stores a **translation key**
and resolves it through `translate` when the model is dumped to JSON:

```python
from pydantic import BaseModel

from fastapi_views.i18n import Translatable
...
!!! note
    `Translatable` resolves bare keys only — it does not interpolate runtime values. For
