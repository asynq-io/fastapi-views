...
problem `type` and `title` are not translated.
...
!!! warning
    `translate` expects a **key**, not a sentence. A key that is missing everywhere
    degrades to [the text after its last `.`](#translation-managers), so passing a
    literal message that contains a period (`_("Item not found.")`) resolves to an empty
    string. Keep literals dot-free, or give every message a key.
...
!!! warning
    The filters read the locale from the context (`get_locale()`), **not** from an explicit
    `locale=` argument to `format_key` / `translate`. `translate("cart.summary",
    locale="pl", count=5)` picks the Polish *text* but formats the numbers with the context
    locale. To format out-of-band, wrap the call in
    [`override_locale`](#accessing-the-current-locale) instead.
