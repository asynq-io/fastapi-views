...
| `CursorPage[T]` | `CursorPaginationFilter` | `items`, `cursor`, `next_page`, `previous_page` |

Both inherit from `BasePage[T]` which provides the `items: list[T]` field.

`CursorPage` cursors are automatically base64-encoded in JSON responses and decoded on input, so consumers see opaque cursor strings while repository implementations can store plain values.
...
| `Cursor` | `str` with base64 encode/decode validators |
