...
from fastapi_views.pagination import CursorPage, decode_cursor, encode_cursor
...
def test_cursor_page_cursor_defaults_to_none():
    page = CursorPage(items=[])
    assert page.cursor is None
    assert page.model_dump(mode="json")["cursor"] is None


def test_cursor_page_cursor_base64_encoded_like_next_page():
    page = CursorPage(items=[], cursor="abc", next_page="abc")
